...
QString Utility::getNumericConfigError(const QJsonObject &config, const QString &key,
                                       const QString &name, int lowerBound, int upperBound)
{
    if (!config.contains(key) || (config[key].isString() && config[key].toString().isEmpty())) {
...
QString
Utility::getStringConfigError(const QJsonObject &config, const QString &key, const QString &name,
                              const QList<std::function<bool(const QString &)>> &checkpoints,
                              bool allowEmpty, const QString &notPassedMsg)
{
...
QString Utility::formatV2RayLog(const QString &log)
{
    ...
    QString time =
            log.mid(timeStart + 6, levelStart - timeStart - 14).replace('-', '/').replace('T', ' ');
...
QString Utility::getLatestRelease(const QString &releaseUrl, const QNetworkProxy *proxy)
{
...
QString Utility::getReleaseAssets(const QString &assetsUrl, const QString &fileName,
                                  const QString &fileExtension, const QString &outputFolderPath,
                                  const QNetworkProxy *proxy)
{
    QByteArray assetsBytes = NetworkRequest::getNetworkResponse(assetsUrl, proxy);
    QString assetsFilePath =
            QDir(Configurator::getAppTempDir()).filePath(QString("Upgrade-%1").arg(fileName));
