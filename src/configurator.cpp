...
QJsonObject Configurator::DEFAULT_APP_CONFIG = { { "autoStart", DEFAULT_AUTO_START },
                                                 { "hideWindow", DEFAULT_HIDE_WINDOW },
                                                 { "enableSysProxy", AUTO_ENABLE_SYS_PROXY },
                                                 { "defaultSysProxyProtocol",
                                                   DEFAULT_SYS_PROXY_PROTOCOL },
...
{
    const static QMap<QLocale::Language, QString> LANGUAGES { { QLocale::Chinese, "zh-CN" } };
...
{
    connectedServerNames = getAutoConnectServers();
}
...
{
    QDir appConfigDir = QDir(QStandardPaths::writableLocation(QStandardPaths::ConfigLocation))
...
QString Configurator::getV2RayInstallDirPath()
{
    ...
    return QDir(QCoreApplication::applicationDirPath()).filePath(V2RAY_LOCAL_INSTALL_DIR);
}
...
QJsonObject Configurator::getAppConfig()
{
    ...
    QString _glu = "https://raw.githubusercontent.com/gfwlist/gfwlist/master/gfwlist.txt";
...
QString Configurator::getLanguage()
{
    ...
    return appConfig.contains("language") ? appConfig["language"].toString() : getDefaultLanguage();
}
...
QJsonArray Configurator::getServers()
{
    QJsonObject appConfig = getAppConfig();
    QJsonArray servers =
            appConfig.contains("servers") ? appConfig["servers"].toArray() : QJsonArray();
    return servers;
}
    ...
    for (auto itr = servers.begin(); itr != servers.end(); ++itr) {
        ...
        if (server.contains("name") && server["name"].toString() == serverName) {
            serverConfig["subscription"] =
                    server.contains("subscription") ? server["subscription"].toString() : "";
            *itr = serverConfig;
            isEdited = true;
        }
    }
...
QMap<QString, QJsonObject> Configurator::removeSubscriptionServers(QString subscriptionUrl)
{
    QJsonArray servers = getServers();
...
{
    static const QStringList SUPPORTED_MATCHES { "DOMAIN-SUFFIX", "DOMAIN",      "DOMAIN-KEYWORD",
    ...
    if (ruleFile.exists() && ruleFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
