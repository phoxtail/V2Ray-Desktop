...
AppProxy::AppProxy(QObject *parent)
    : QObject(parent), v2ray(V2RayCore::getInstance()), configurator(Configurator::getInstance())
{
    // Setup Worker
    ...
    connect(this, &AppProxy::getServerLatencyStarted, worker, &AppProxyWorker::getServerLatency);
    connect(worker, &AppProxyWorker::serverLatencyReady, this, &AppProxy::returnServerLatency);

    // Setup Worker -> getGfwList
    connect(this, &AppProxy::getGfwListStarted, worker, &AppProxyWorker::getGfwList);
    connect(worker, &AppProxyWorker::gfwListReady, this, &AppProxy::returnGfwList);

    // Setup Worker -> getNetworkStatus
    connect(this, &AppProxy::getNetworkStatusStarted, worker, &AppProxyWorker::getUrlAccessibility);
    ...
    connect(this, &AppProxy::getLatestReleaseStarted, worker, &AppProxyWorker::getLatestRelease);
    connect(worker, &AppProxyWorker::latestReleaseReady, this, &AppProxy::returnLatestRelease);

    // Setup Worker -> upgradeDependency
    connect(this, &AppProxy::upgradeStarted, worker, &AppProxyWorker::upgradeDependency);
    connect(worker, &AppProxyWorker::upgradeFinished, this, &AppProxy::replaceDependency);
...
QString AppProxy::getAppVersion()
{
    QString appVersion =
            QString("v%1.%2.%3")
                    .arg(QString::number(APP_VERSION_MAJOR), QString::number(APP_VERSION_MINOR),
...
void AppProxy::setV2RayCoreRunning(bool expectedRunning)
{
    ...
    if (expectedRunning) {
        ...
        qInfo() << QString("Start Clash ... %1").arg(isSuccessful ? "success" : "failed");
    } else {
        isSuccessful = v2ray.stop();
        qInfo() << QString("Stop Clash ... %1").arg(isSuccessful ? "success" : "failed");
    }
    if (isSuccessful) {
...
void AppProxy::getNetworkStatus()
{
    ...
    emit getNetworkStatusStarted({ { "google.com", true }, { "baidu.com", false } }, getQProxy());
}
...
{
    bool isGoogleAccessible = accessible.contains("google.com") ? accessible["google.com"] : false,
         isBaiduAccessible = accessible.contains("baidu.com") ? accessible["baidu.com"] : false;

    emit networkStatusReady(QJsonDocument(QJsonObject {
...
{
    QStringList errors;
    errors.append(Utility::getStringConfigError(appConfig, "language", tr("Language")));
    ...
    errors.append(Utility::getNumericConfigError(appConfig, "httpPort", tr("HTTP Port"), 1, 65535));
    errors.append(
            Utility::getNumericConfigError(appConfig, "socksPort", tr("SOCKS Port"), 1, 65535));
...
bool AppProxy::retranslate(QString language)
{
    ...
    bool isTrLoaded =
            translator.load(QString("%1/%2.qm").arg(Configurator::getLocaleDirPath(), language));
...
void AppProxy::setAutoStart(bool autoStart)
{
    ...
    const QString APP_PATH = QDir::toNativeSeparators(Configurator::getAppFilePath());
#if defined(Q_OS_WIN)
    ...
    QFile dstFile(QString("%1/Library/LaunchAgents/com.v2ray.desktop.launcher.plist")
                          .arg(QDir::homePath()));
#endif
...
{
    emit getLogsStarted(Configurator::getAppLogFilePath(), Configurator::getV2RayLogFilePath());
}

void AppProxy::returnLogs(QString logs)
{
    emit logsReady(logs);
}
...
void AppProxy::returnGfwList(QString gfwList)
{
    if (gfwList.size()) {
        ...
        configurator.setAppConfig(
                QJsonObject { { "gfwListLastUpdated", QDateTime::currentDateTime().toString() } });
...
void AppProxy::getServers()
{
    ...
    for (auto itr = servers.begin(); itr != servers.end(); ++itr) {
        ...
        QString serverName = server.contains("name") ? server["name"].toString() : "";
...
{
    ServerConfigHelper::Protocol _protocol = ServerConfigHelper::getProtocol(protocol);
    QJsonDocument configDoc = QJsonDocument::fromJson(configString.toUtf8());
    ...
    // Save server config
    configurator.addServer(ServerConfigHelper::getPrettyServerConfig(_protocol, serverConfig));
    emit serversChanged();
...
void AppProxy::addSubscriptionUrl(QString subsriptionUrl)
{
    QString error = Utility::getStringConfigError(
            { { "subsriptionUrl", subsriptionUrl } }, "subsriptionUrl", tr("Subscription URL"),
            { std::bind(&Utility::isUrlValid, std::placeholders::_1) });
    if (!error.isEmpty()) {
...
void AppProxy::addSubscriptionServers(QString subsriptionServers, QString subsriptionUrl)
{
    ...
    for (QString server : servers) {
        ...
        QJsonObject serverConfig =
                ServerConfigHelper::getServerConfigFromUrl(protocol, server, subsriptionUrl);
        QStringList serverConfigErrors =
                ServerConfigHelper::getServerConfigErrors(protocol, serverConfig);
        serverConfig = ServerConfigHelper::getPrettyServerConfig(protocol, serverConfig);
        ...
        QString serverName = serverConfig.contains("name") ? serverConfig["name"].toString() : "";
        serverConfig["autoConnect"] = removedServers.contains(serverName)
                ? removedServers[serverName]["autoConnect"].toBool()
                : false;
        // Save the server
        configurator.addServer(serverConfig);
        qInfo() << QString("Add a new server[Name=%1] from URI: %2").arg(serverName, server);
        ++nImportedServers;
    }
    if (nImportedServers) {
        emit serversChanged();
...
void AppProxy::addServerConfigFile(QString configFilePath, QString configFileType)
{
    ...
    if (configFileType == "v2ray-config") {
        protocol = ServerConfigHelper::Protocol::VMESS;
        servers = ServerConfigHelper::getServerConfigFromV2RayConfig(configDoc.object());
    } else if (configFileType == "shadowsocks-qt5-config") {
        protocol = ServerConfigHelper::Protocol::SHADOWSOCKS;
        servers = ServerConfigHelper::getServerConfigFromShadowsocksQt5Config(configDoc.object());
    }
    for (QJsonObject server : servers) {
        QStringList serverConfigErrors =
                ServerConfigHelper::getServerConfigErrors(protocol, server);
        if (!serverConfigErrors.empty()) {
            qWarning() << QString("Error occurred for the server[Name=%1]. Errors: %2")
                                  .arg(server["name"].toString(), serverConfigErrors.join(" "));
            continue;
        } else {
            configurator.addServer(ServerConfigHelper::getPrettyServerConfig(protocol, server));
            qInfo() << QString("Add a new server[Name=%1] from Shadowsocks-Qt5 config file.")
    ...
    if (nAddServers) {
        ...
        emit serverConfigError(tr("No supported servers added from the config file."));
    }
}

void AppProxy::editServer(QString serverName, QString protocol, QString configString)
{
    QJsonDocument configDoc = QJsonDocument::fromJson(configString.toUtf8());
    ServerConfigHelper::Protocol _protocol = ServerConfigHelper::getProtocol(protocol);
    QJsonObject serverConfig = configDoc.object();
    QStringList serverConfigErrors =
            ServerConfigHelper::getServerConfigErrors(_protocol, serverConfig, &serverName);
    if (serverConfigErrors.size() > 0) {
        emit serverConfigError(serverConfigErrors.join('\n'));
        return;
    }
    serverConfig = ServerConfigHelper::getPrettyServerConfig(_protocol, serverConfig);
...
void AppProxy::removeSubscriptionServers(QString subscriptionUrl)
{
    ...
    qInfo() << QString("Servers from Subscription %1 have been removed.").arg(subscriptionUrl);
    emit serversChanged();
}
...
void AppProxy::scanQrCodeScreen()
{
    ...
    for (int i = 0; i < screens.size(); ++i) {
        ...
        QPixmap screenshot = screens.at(i)->grabWindow(0, r.x(), r.y(), r.width(), r.height());
    ...
    qInfo() << QString("Add %1 servers from QR code.").arg(QString::number(servers.size()));
...
void AppProxy::getLatestRelease(QString name)
{
    ...
    if (latestVersion[name]["checkTime"].toDateTime().secsTo(QDateTime::currentDateTime())
        < RELEASE_CHECK_INTERVAL) {
        emit latestReleaseReady(name, latestVersion[name]["latestVersion"].toString());
        return;
    }
    if (name == "v2ray-core") {
...
void AppProxy::returnLatestRelease(QString name, QString version)
{
    ...
    if (!Utility::isVersionNewer(latestVersion[name]["currentVersion"].toString(), version)) {
        version = "";
    }
...
void AppProxy::upgradeDependency(QString name, QString version)
{
    if (!V2RAY_USE_LOCAL_INSTALL) {
        ...
        emit upgradeError(name, tr("The V2Ray Core in AppImage is not upgradable."));
        return;
    }
    if (name == "v2ray-core") {
#if defined(Q_OS_WIN)
...
#endif
        emit upgradeStarted(name,
                            APP_ASSETS_URL.arg(version, version, operatingSystem, fileExtension),
                            Configurator::getAppTempDir().filePath(name), getQProxy());
    }
}

void AppProxy::replaceDependency(QString name, QString outputFilePath, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        if (name == "v2ray-core") {
            v2ray.stop();
            if (!replaceV2RayCoreFiles(Configurator::getV2RayInstallDirPath(), outputFilePath)) {
...
bool AppProxy::replaceV2RayCoreFiles(const QString &srcFolderPath, const QString &dstFolderPath)
{
#if defined(Q_OS_WIN)
    ...
    QFile(v2RayExecFilePath)
            .setPermissions(QFileDevice::ReadUser | QFileDevice::WriteOwner | QFileDevice::ExeUser);
