...
#elif defined(Q_OS_MAC)
const QStringList NetworkProxyHelper::NETWORK_INTERFACES = { "Wi-Fi", "Enternet" };
...
void NetworkProxyHelper::setSystemProxyMacOs(const NetworkProxy &proxy)
{
    ...
    static const QMap<QString, QString> SETTING_KEYS = { { "http", "web" },
                                                         { "socks", "socksfirewall" } };
    ...
    if (proxy.getMode() == NetworkProxyMode::GLOBAL_MODE) {
        ...
        for (QString ni : NETWORK_INTERFACES) {
            p.start("networksetup",
                    QStringList { QString("-set%1proxy").arg(settingKey), ni, proxy.getHost(),
...
void NetworkProxyHelper::resetSystemProxyMacOs()
{
    for (QString ni : NETWORK_INTERFACES) {
        ...
        p.waitForFinished();
        p.start("networksetup", QStringList { "-setsocksfirewallproxystate", ni, "off" });
        p.waitForFinished();
    }
}
#elif defined(Q_OS_LINUX)
NetworkProxy NetworkProxyHelper::getSystemProxyLinuxGnome()
{
    QProcess p;
    NetworkProxy proxy;
    p.start("gsettings", QStringList { "list-recursively", "org.gnome.system.proxy" });
    p.waitForFinished();
...
void NetworkProxyHelper::setSystemProxyLinuxGnome(const NetworkProxy &proxy)
{
    if (proxy.getMode() == NetworkProxyMode::GLOBAL_MODE) {
        QProcess p;
        p.start("gsettings", QStringList { "set", "org.gnome.system.proxy", "mode", "manual" });
        p.waitForFinished();
        p.start("gsettings",
                QStringList { "set", QString("org.gnome.system.proxy.%1").arg(proxy.getProtocol()),
                              "host", proxy.getHost() });
        p.waitForFinished();
        p.start("gsettings",
                QStringList { "set", QString("org.gnome.system.proxy.%1").arg(proxy.getProtocol()),
...
{
    QProcess p;
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy", "mode", "none" });
    p.waitForFinished();
    p.start("gsettings",
    ...
    p.waitForFinished();
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy.http", "host", "" });
    p.waitForFinished();
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy.http", "port", "0" });
    p.waitForFinished();
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy.socks", "host", "" });
    p.waitForFinished();
    p.start("gsettings", QStringList { "set", "org.gnome.system.proxy.socks", "port", "0" });
    p.waitForFinished();
}
...
NetworkProxy NetworkProxyHelper::getSystemProxyLinuxKde()
{
    ...
    p.start("kreadconfig5",
            QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key",
                          "ProxyType" });
    p.waitForFinished();
    ...
    if (proxy.getMode() == NetworkProxyMode::GLOBAL_MODE) {
        ...
        p.start("kwriteconfig5",
                QStringList {
                        ...
                        QString("%1Proxy").arg(proxy.getProtocol()),
                        QString("%1 %2").arg(proxy.getHost(), QString::number(proxy.getPort())) });
        p.waitForFinished();
    }
}
...
void NetworkProxyHelper::resetSystemProxyLinuxKde()
{
    QProcess p;
    p.start("kwriteconfig5",
            QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key", "ProxyType",
                          "0" });
    p.waitForFinished();
    p.start("kwriteconfig5",
            QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key",
    ...
    p.waitForFinished();
    p.start("kwriteconfig5",
            QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key", "httpProxy",
                          "" });
    p.waitForFinished();
    p.start("kwriteconfig5",
            QStringList { "--file", "kioslaverc", "--group", "Proxy Settings", "--key",
