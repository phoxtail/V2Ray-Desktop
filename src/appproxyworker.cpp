...
void AppProxyWorker::getServerLatency(QJsonArray servers)
{
    ...
    for (auto itr = servers.begin(); itr != servers.end(); ++itr) {
        ...
        int latency =
                NetworkRequest::getLatency(server["server"].toString(), server["port"].toInt());
...
{
    QNetworkProxy *p = proxy.type() == QNetworkProxy::ProxyType::NoProxy ? nullptr : &proxy;
    QString gfwList = NetworkRequest::getNetworkResponse(gfwListUrl, p);
    emit gfwListReady(gfwList);
}

void AppProxyWorker::getUrlAccessibility(QMap<QString, bool> urls, QNetworkProxy proxy)
{
    // Ref:
    ...
    for (auto l : urls.toStdMap()) {
        ...
        QByteArray response = NetworkRequest::getNetworkResponse(QString("http://www.%1").arg(url),
                                                                 p, HTTP_GET_TIMEOUT);
...
void AppProxyWorker::getSubscriptionServers(QString subscriptionUrl, QNetworkProxy proxy)
{
    QNetworkProxy *p = proxy.type() == QNetworkProxy::ProxyType::NoProxy ? nullptr : &proxy;
...
void AppProxyWorker::getLogs(QString appLogFilePath, QString v2RayLogFilePath)
{
    ...
    if (appLogFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        int cnt = 0;
        for (auto itr = _logList.end() - 1; itr >= _logList.begin() && cnt <= MAX_N_LOGS;
             --itr, ++cnt) {
            logs.append(*itr);
        }
        appLogFile.close();
...
void AppProxyWorker::getLatestRelease(QString name, QString releaseUrl, QNetworkProxy proxy)
{
    QNetworkProxy *p = proxy.type() == QNetworkProxy::ProxyType::NoProxy ? nullptr : &proxy;
...
void AppProxyWorker::upgradeDependency(QString name, QString assetsUrl, QString outputFolderPath,
                                       QNetworkProxy proxy)
{
    ...
    QNetworkProxy *p = proxy.type() == QNetworkProxy::ProxyType::NoProxy ? nullptr : &proxy;
    QString errorMsg =
            Utility::getReleaseAssets(assetsUrl, fileName, fileExtension, outputFolderPath, p);
